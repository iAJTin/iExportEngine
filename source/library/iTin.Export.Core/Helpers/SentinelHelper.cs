...
namespace iTin.Export.Helpers
{
    ...
    public static class SentinelHelper
    {
        ...
        public static bool IsEnumValid<T>(T value, bool testOnly) where T : struct
        {
            if (!typeof(T).IsEnum)
            {
                throw new ArgumentException(Resources.ErrorMessage.EnumArgumentException);
            }
            ...
            if (!found)
            {
                var message = string.Format(CultureInfo.InvariantCulture, Resources.ErrorMessage.InvalidEnumArgumentException, value, typeof(T));
